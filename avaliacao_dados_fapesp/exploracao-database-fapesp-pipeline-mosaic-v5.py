"""
=============================================================================
PIPELINE DE DESCOBERTA DE CLASSES v5 — MOSAIC-FL
COMPARADOR DE ESQUEMAS DE CLASSIFICAÇÃO (placar com 3 critérios)
=============================================================================

EVOLUÇÃO v2 -> v5:
    v2: 1 linha/exame (viés de duplicação).
    v3: agrega por atendimento (corrige duplicação; viés residual na amostragem).
    v4: amostragem uniforme por atendimento (resumos completos).
    v5: sobre a base correta do v4, COMPARA vários esquemas de classes para
        descobrir se existe um melhor que as 5 classes teóricas atuais.

OBJETIVO:
    As 5 classes atuais (curado_pronto, curado_internado, melhora_pronto,
    melhora_internado_breve, melhora_internado_grave) vêm de REGRA CLÍNICA, têm
    classes raríssimas (melhora_pronto ~0,2%) e são heterogêneas entre hospitais
    — o que derruba a acurácia sob privacidade diferencial. O v5 gera esquemas
    CANDIDATOS e os pontua num PLACAR de 3 critérios, para decidir com evidência.

ESQUEMAS CANDIDATOS COMPARADOS (Parte 5):
    E0 — 5 classes ATUAIS (baseline, regra da documentação, corte 10 dias)
    E1 — Cortes de duração alternativos (5/7/10/14/21 dias) na mesma regra
    E2 — Clustering puro (k-prototypes propõe grupos do zero, k=2..8)
    E3 — Esquema que INCORPORA os ~47% 'outro' (outcome 2-6, hoje descartados)

PLACAR — 3 critérios por esquema:
    [1] Balanceamento : tamanho da MENOR classe (%). Maior = melhor
                        (classes raras somem sob ruído DP).
    [2] Equilíbrio hosp: variação média das classes entre hospitais.
                        Menor = melhor (menos non-IID, melhor federação).
    [3] Poder preditivo: alinhamento classe<->outcome_class. Maior = melhor.

AMOSTRAGEM:
    Estratificada e BALANCEADA por hospital: mesma quantidade de atendimentos
    de cada hospital (limitada pelo hospital com MENOS dados), para o critério
    de equilíbrio entre hospitais ser mensurável e justo.

ESTRUTURA:
    Parte 0 — Viés de granularidade (herdada do v4)
    Parte 1 — Exploração (relatório visual)
    Parte 2 — Clusterização (k-prototypes)
    Parte 3 — Auditoria de qualidade
    Parte 4 — Análises (duração, hospitais, classes atuais)
    Parte 5 — PLACAR COMPARATIVO de esquemas  <-- novidade do v5

Saídas versionadas (VERSÃO + data/hora):
    vies_V5_...txt, relatorio_agg_V5_...html, clusters_agg_V5_...csv,
    auditoria_agg_V5_...txt, analises_agg_V5_...txt,
    placar_esquemas_V5_...txt   <-- novo
...
# VERSIONAMENTO
# =============================================================================
VERSAO = "V5"    # identifica qual pipeline gerou o arquivo (ex: placar_esquemas_V5_2026-07-06_1145.txt)
CARIMBO = datetime.now().strftime("%Y-%m-%d_%H%M")
print(f"Execução {VERSAO} (comparador de esquemas de classes): {CARIMBO}\n")
...
def carregar_longo():
    ...
    if ESTRATIFICAR_HOSPITAL:
        ...
        # 'por_hospital' linhas de CADA hospital, todos contribuem igualmente.
        # 'por_hospital' é limitado pelo hospital com MENOS atendimentos elegíveis,
        # calculado em tempo real com uma janela sobre a contagem mínima.
        n_hosp_estimado = 2  # BPSP e HSL; se houver mais, o LIMIT por hospital ajusta
        por_hospital = LIMITE_ATENDIMENTOS // n_hosp_estimado
        cte_amostra = f"""
        WITH elegiveis AS (
        ...
        limite AS (
            -- menor hospital define quantos pegar de cada (mín entre por_hospital
            -- e o tamanho do menor hospital), garantindo balanceamento perfeito
            SELECT LEAST({por_hospital}, MIN(n_hosp)) AS por_hosp FROM elegiveis
        ),
        amostra AS (
